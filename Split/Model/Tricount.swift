...
func getInfoFromTricount(tricountID: String) async throws -> Tricount{
    ...
    await webViewStore.webView.load(URLRequest(url: tricountLink))
    
    try await Task.sleep(nanoseconds: UInt64(2 * Double(NSEC_PER_SEC)))
    
    // Conitnuations help from:
    ...
    let tricountName: String = await withCheckedContinuation { continuation in
        ...
        Task{
            await webViewStore.webView.evaluateJavaScript(getTricountName) { res, error in
                if let result = res {
    ...
    let names: [String] = await withCheckedContinuation { continuation in
        ...
        Task{
            await webViewStore.webView.evaluateJavaScript(getNames) { res, error in
                if let result = res {
